...
class ArticleModel {
  public:
    ...
    * @return std::tuple<oatpp::Object<ArticleDto>, std::string, std::string, std::string> This returns article object, article id, author user id, tags json in string. Remember to append data for favourite, taglist and author profile.
    */
    std::tuple<oatpp::Vector<oatpp::Object<ArticleDto>>, std::vector<std::string>, std::vector<std::string>, std::vector<std::string>> getArticles(unsigned int limit, unsigned int offset, std::string &tagId, std::string &authorId, std::vector<std::string> &favouriteArticles);
